  ...
  ArrayBufferDataStream.prototype.writeUnsignedIntBE = function (u, width) {
    ...
    switch (width) {
      case 5:
        ...
        ) // Need to use division to access >32
...
(function () {
  const BlobBuffer = function (fs) {
    return function (destination) {
      const buffer = []; let writePromise = Promise.resolve(); let fileWriter = null
      let fd = null
      ...
      function convertToUint8Array(thing) {
        return new Promise((resolve, reject) => {
          if (thing instanceof Uint8Array) {
            ...
            resolve(readBlobAsBuffer(thing).then((buffer) => {
              return new Uint8Array(buffer)
            }))
          }
          else {
            // Assume that Blob will know how to read this thing
            resolve(readBlobAsBuffer(new Blob([thing])).then((buffer) => {
              return new Uint8Array(buffer)
            }))
          }
        })
      ...
      this.write = function (data) {
        const newEntry = { offset: this.pos, data, length: measureData(data) }
        ...
        writePromise = writePromise.then(async () => {
          if (fd) {
            return new Promise((resolve, reject) => {
              convertToUint8Array(newEntry.data).then((dataArray) => {
                let totalWritten = 0; const buffer = Buffer.from(dataArray.buffer)
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined')
    module.exports = BlobBuffer(require('node:fs'))
  else
...
(function () {
  ...
  function byteStringToUint32LE(string) {
    const a = string.charCodeAt(0); const b = string.charCodeAt(1)
    const c = string.charCodeAt(2); const d = string.charCodeAt(3)
  ...
  function writeEBML(buffer, bufferFileOffset, ebml) {
    ...
    if (Array.isArray(ebml)) {
      ...
      if (Array.isArray(ebml.data)) {
        ...
        throw new TypeError(`Bad EBML datatype ${typeof ebml.data}`)
      }
    }
    else {
   ...
   * @constructor
   */
  const WebMWriter = function (ArrayBufferDataStream, BlobBuffer) {
    return function (options) {
      const MAX_CLUSTER_DURATION_MSEC = 5000; const DEFAULT_TRACK_NUMBER = 1
      let writtenHeader = false; let videoWidth = 0; let videoHeight = 0
      let firstTimestampEver = true; let earliestTimestamp = 0

      /**
       *
       * @type {Frame[]}
       */
      let clusterFrameBuffer = []; let clusterStartTime = 0; let clusterDuration = 0
      let lastTimeCode = 0
      ...
      function createSeekHead() {
        ...
        for (const name in seekPoints) {
          const seekPoint = seekPoints[name]
      ...
      function writeHeader() {
        ...
        const bufferStream = new ArrayBufferDataStream(256)
      ...
      function createSimpleBlockForframe(frame) {
        ...
        bufferStream.writeEBMLVarInt(
          frame.trackNumber,
        ) // Always 1 byte since we limit the range of
        // trackNumber
      ...
      function writeCues() {
        ...
        const ebml = { id: 0x1C53BB6B, data: cues }

        const cuesBuffer = new ArrayBufferDataStream(
          16
          + cues.length
          * 32,
      ...
      function rewriteSeekHead() {
        const seekHeadBuffer = new ArrayBufferDataStream(seekHead.size)
      ...
      function rewriteDuration() {
        const buffer = new ArrayBufferDataStream(8); const oldPos = blobBuffer.pos
      ...
      this.addFrame = function (frame) {
        ...
        if (frame.constructor.name == 'EncodedVideoChunk') {
          const frameData = new Uint8Array(frame.byteLength)
